...
<h1><?php echo htmlspecialchars($student_data['name']); ?>'s Courses</h1>
            ...
            <?php if (isset($courses) && $courses->num_rows > 0): ?>
                <?php while ($row = $courses->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td>
                            <form action="/student/remove-course" method="post" class="d-inline" onsubmit="return confirm('Are you sure you want to remove this course?')">
                            ...
                            <?php if (!$row['grade']): ?>
                            <a href="/student/update-grade/<?php echo $student_data['id']; ?>/<?php echo $row['id']; ?>" class="btn btn-warning btn-sm">Add Grade</a>
                            <?php else: ?>
                            <a href="/student/update-grade/<?php echo $student_data['id']; ?>/<?php echo $row['id']; ?>" class="btn btn-info btn-sm">Update Grade</a>
                            <?php endif; ?>
                        </td>
                    </tr>
                <?php endwhile; ?>
